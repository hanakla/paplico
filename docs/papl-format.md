...
Paplico プロジェクトファイル `.papf` のフォーマット仕様。

`.papf` は 2 層でできている。

- **外側: PDF コンテナ**。ディスク上のファイルは正当な PDF で、どの PDF ビューアでも開ける。アートボードごとに 1 ページの JPEG プレビューを持つ
  - PDF にしている理由は、Finder や Explorer のプレビュー、ファイルビューア、ファイル共有サービスなどで中身を確認できるようにするため
  - 印刷用途は現状では想定していない。プレビューは 72 dpi 固定の `/DeviceRGB` で描いている
- **内側: papf バイト列**。Paplico が読み書きする本体。PDF の `/PieceInfo` ストリームに無加工で入っている

この文書の「ファイル構造」で両層の関係を示し、「バイナリ構造」以降は内側の papf バイト列だけを扱う。
IndexedDB への自動保存は内側の papf バイト列を PDF に包まずそのまま保存する。
...
- **PDF 互換**: 保存ファイルはファイルビューアでプレビューでき、Paplico はその中から本体を無加工で取り出せる。Illustrator の「PDF 互換ファイル」と同じ方式。目的はプレビューであり、印刷用途は現状では想定していない
...
%PDF-1.7
├─ Catalog
│    /Pages       → アートボードごとに 1 ページ。JPEG (/DCTDecode) を全面に描く
│    /PieceInfo << /Paplico << /LastModified (D:...) /Private <stream> >> >>
│                                                        └─ papf バイト列 (下記) をそのまま格納
├─ xref
└─ trailer
```

- `/PieceInfo` は ISO 32000 14.5 が定めるアプリ固有データ用の辞書。Illustrator と同じ仕組み
- `/Private` はフィルタ無しの生ストリーム。中身は `serializeDocument()` の出力と完全に一致する
- `/LastModified` は仕様上必須。他アプリが後から編集したかを判定する材料になる
...
### PDF コンテナから papf バイト列を取り出す (`openPapfContainer()`)

```
1. 先頭 5 バイトを読む
2. "PAPF" なら PDF に包まれていない papf バイト列。そのまま次のフローへ
3. "%PDF-" なら @cantoo/pdf-lib で解析し Catalog → PieceInfo → Paplico → Private を辿る
4. /Filter が付いていれば復号する。他アプリでの再保存で Flate が掛かることがある
5. 取り出したバイト列を Blob にして次のフローへ
```

- どちらのマジックでもなければ `PAPF_INVALID_FILE`
- PieceInfo が辿れなければ `PAPF_NOT_FOUND_IN_PDF`。他アプリで再保存されて Paplico データが落ちた場合に起きる

### papf バイト列を読む (`openPapf()`)
...
### papf バイト列を書く (`serializeDocument()`)
...
8. 全チャンクから Blob を構築する
```

IndexedDB への自動保存はこの Blob をそのまま保存する。

### PDF コンテナに包む (`wrapPapfInPdf()`)

ファイルとして保存するときだけ行う。

```
1. 各アートボードを JPEG に描画する。描画に失敗したアートボードは空白ページにして保存を続行する
2. アートボードごとにページを追加し、JPEG を全面に描く
3. papf バイト列を無フィルタのストリームとして登録し、Catalog の /PieceInfo /Paplico /Private から参照する
4. オブジェクトストリームを使わずに PDF を書き出す
```

PDF に包まれていない papf バイト列で保存されていたファイルも、上書き保存すると PDF コンテナになる。
