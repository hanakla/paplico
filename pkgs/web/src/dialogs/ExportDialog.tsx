...
export const ExportDialog = memo(function ExportDialog({
	...
	/**
	 * Runs the format the user picked over every selected artboard, writing
	 * each one out under its own name. One artboard failing leaves the rest to
	 * finish.
	 */
	...
	const buildArtboardRenderer = useEventCallback(
		async (
			...
			if (format === "tiff") {
				// Empty value = RGB TIFF without ICC; the exporter picks RGB/CMYK
				// output from the resolved profile's color space.
									...
									>
										<div className="relative w-full aspect-square rounded bg-foreground/5 border border-border/20 flex items-center justify-center overflow-hidden">
											...
											<Checkbox
												...
												onCheckedChange={() =>
													handleToggleArtboard(artboard.id)
												}
...
/** Names an exported image after its artboard, keeping the name file safe. */
function exportFilename(
	...
	if (!artboard) return `export.${extension}`;

	return `${artboard.name.replace(/[^a-zA-Z0-9-_]/g, "_")}.${extension}`;
}
