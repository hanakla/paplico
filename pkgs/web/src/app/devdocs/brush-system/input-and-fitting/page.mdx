...
このページは、ブラウザのPointer Eventが `CubicBezierSegment[]` へ変換されるまでの現行処理を説明する。対象はブラシツールの中心線である。Dabの生成、幅の焼き込み、Yjsへの保存は扱わない。
...
| raw pointsと増分fitの状態 | `pkgs/web/src/core/renderer/canvas/pipeline/stroke/BrushStrokeSession.ts` |
