...
各raw pointはワールド座標、pressure、tiltX、tiltY、twist、ストローク開始からの `deltaTime` を持つ。coalesced sampleの時刻には `sample.timeStamp - startTime` を使う。coalesced sampleがない場合は `event.timeStamp - startTime` を使う。
...
`PenTool` は `IncrementalStrokeFitter.getPlainSegments` からPathを作る。frozen prefixとtailのfitはプレビューと同じで、input knotだけを含まない。幅プロファイルとアピアランスはプレビューと同じ値を保存する。
