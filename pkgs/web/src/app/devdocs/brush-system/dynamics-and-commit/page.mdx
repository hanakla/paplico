...
速度入力を計算するのは現行のdab engineである。ribbon engineのcurve評価はpressure、`strokeT`、fadeを設定する。`speedFine` と `speedGross` はneutral値の0に残る。geometric engineは専用のpressure幅式を使う。
...
現行のdab engineに `sizeBySpeed` と `pooling` の専用分岐はない。v1のscatter brushとcalligraphy brushをv2へ読み込むとき、これらの値を `speedFine` 入力のproperty curveへ変換する。
