...
> [!NOTE]
> このページは[パイプライン概要](/devdocs/renderer/pipeline)の内容を前提とする。`CanvasLayer` が `buildFramePlanStructure()` と `buildFramePlanView()` を呼び出す流れを先に把握しておくと理解しやすい。
...
`buildFramePlanView()` が返すFramePlanは、1フレームの描画に必要な全情報を含む:
...
CanvasLayer はこの FramePlan に、マスクの適用計画（`maskApplicationPlans`）とグループの合成計画（`groupCompositionPlans`）を足して使う。どちらもマスクの事前パスが埋める。詳細は[クリッピング方式](/devdocs/renderer/clipping)を参照。

## 構築の流れ

計画は2つの関数で作られる。

- `buildFramePlanStructure()` — ドキュメントだけから決まる部分（`FramePlanStructure`）を作る。下の段階1〜3がこれに当たる。ビューポートに依存しないため、CanvasLayer は `document.objects`・`layers`・`artboards`・`appearancePresets` と transient 要素の参照が変わるまで結果を再利用する。ツールのオーバーライド（`elementOverrides`）があるフレームは再利用しない。
- `buildFramePlanView()` — 毎フレーム、`FramePlanStructure` の候補をビューポートでカリングし、段階4のセグメント分割を行って `FramePlan` を返す。コストは O(候補数 + レイヤー数) で、全要素の走査を含まない。
...
`collectPlanCandidates()` が各レイヤーの要素を走査し、要素ごとのフィルターを分類する。これがRenderPlannerの中核処理である。分類結果は、ワールド空間のバウンズを添えた候補（`PlanCandidate`）として `FramePlanStructure` に積まれる。グループ自身がフィルター計画を持つ場合は子へ再帰しない。その子のフィルターは、グループをテクスチャに描く経路が処理する。Repeat 要素では、レイヤーに属さないソース要素もここで分類する。
...
フィルターハンドラーの `getRenderConfigure()` が返す `needsBackdrop` でバックドロップフィルター（frost-glass等）と通常フィルターを分離する。フィルターに `applyToBackdrop` が付いている場合は、`postProcess` を持つフィルターもバックドロップ側に入る。要素の描画を置き換えるアピアランス（ガラスの押し出し）は、`needsBackdrop` であっても通常フィルター側に残る。このアピアランスはメインパスの途中で自前の合成を行うためである。バックドロップフィルターを持つ要素は `BackdropElementEntry` として記録され、通常の要素描画フローから外れる。

フィルターの段は `classifyFilterHandler()` が決める。`preProcess` または `preProcessAppearance` を持つハンドラーは geometry、`postProcess` を持つハンドラーは raster である。

#### Expansion Marginの計算

オフスクリーン描画が必要な要素について、フィルターハンドラーの `getExpansionMargin()` から必要な拡張量を取得し、要素バウンズを拡張してテクスチャサイズ（`textureBounds`）を決定する。ぼかしフィルターなど、要素の境界を超えて描画効果が及ぶフィルターに対応するためである。wet が有効なストロークでは、顔料がアウトラインの外へにじむ距離もここに加える。

#### AppearancePlanの生成

次のいずれかに当たる要素は、アピアランスごとに個別のオフスクリーン描画が必要になる。

- アピアランス（fill/stroke）にラスターのサブフィルターがある
- path でも group でもない要素のアピアランスに、ジオメトリのサブフィルター（zigzag変形等）がある。path と group はジオメトリのサブフィルターをインラインで変形するため、これだけではオフスクリーンにならない
- グループ以外の要素で、アピアランスに非normalブレンドモードが指定されている
- 単独のテクスチャに分離して描く必要があるストローク（wash ストローク）がある

`AppearancePlan` はアピアランス単位の描画計画であり、プリサブフィルター（ジオメトリ変形）とポストサブフィルター（画像処理）を分離して保持する。wash ストロークでは、合成時に1回だけ掛けるストローク不透明度（`washStrokeOpacity`）、wet edge の設定（`washWetEdge`）、ブラシサイズ（`washBrushSize`）も保持する。

### 4. レイヤーセグメントの構築

`buildFramePlanView()` は候補のバウンズをビューポートと照合し、交差する候補のフィルター計画とバックドロップエントリだけを採用する。続いて `buildLayerSegments()` が各レイヤーの要素列を、採用されたバックドロップ要素の位置で分割し、`LayerSegment` の配列を生成する。
...
| フィールド | 説明 |
|---|---|
| `layerIndex` / `layerId` | ドキュメント内でのレイヤーの位置とID |
...
| フィールド | 説明 |
|---|---|
| `elementId` / `element` | 対象の要素 |
| `bounds` | 要素のワールド空間バウンズ |
...
| `allAppearancePlans` | アピアランスごとの個別描画計画（サブフィルター、非normalブレンド、wash ストロークのいずれかがあるときのみ） |

## 設計上の判断

### 純粋関数としての実装

RenderPlannerはクラスではなく、`buildFramePlanStructure()` と `buildFramePlanView()` を頂点とする純粋関数群として実装されている。GPUリソースへの参照を持たず、フィルターハンドラーのメタデータ（`getRenderConfigure()` の `needsBackdrop`, `getExpansionMargin()`, `preProcess`/`postProcess` の有無）だけを参照する。
...
分類はカリングせずに全要素に対して行い、`buildFramePlanView()` が候補単位で `boundsIntersectViewport()` を適用する。画面外の要素のフィルター計画を採用するとオフスクリーンテクスチャの無駄な確保につながるため、この段階で除外する。分類をビューポートから切り離してあるので、パンやズームのフレームは分類をやり直さない。Repeat のソース要素のフィルター計画は `skipCull` でカリングの対象外になる。Repeat はビューポートに関係なく全インスタンスをベイクするためである。

レイヤーに属さない要素のフィルター計画は `buildFilterPlansForElements()` が作る。オブジェクトマスクの中身がこれに当たる。マスクの中には背後のドキュメントが無いので、この経路はバックドロップフィルターを扱わない。

### パス中断の事前宣言

`buildPassPlan()` は `LayerSegment` をさらに細かく切り、メインパスを中断する位置を宣言する。中断の種類（`SegmentBreak`）は3つある。

- `backdrop` — バックドロップ要素の処理
- `compositeElement` — 非normalの blendMode または compositionMode を持つ要素の合成
- `inlineBackdropCompose` — ガラスの押し出しのインライン合成

中断を起こす要素は `elements` ではなく `breakAfter` に入る。ここではビューポートカリングを適用しない。

> [!TIP]
> 毎フレーム実行される `buildFramePlanView()` の構築コストは小さい。全処理がCPU上の配列走査とMap操作であり、GPUコマンドの発行やバッファ書き込みは一切行わない。実際のGPUリソース確保は `CanvasLayer` がFramePlanを消費する段階で行われる。
