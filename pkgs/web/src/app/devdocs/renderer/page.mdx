...
| [caches/](/devdocs/renderer/caches) | RenderCacheManager, GeometryCache 等 | テッセレーション（曲線パスを描画可能なジオメトリに変換する処理）・テクスチャ・BindGroup（GPUリソースをシェーダーにバインドする単位）の再利用キャッシュ |
...
| geometry/ | strokeTessellator, CornerRadiusProcessor | パステッセレーション、角丸処理 |
...
- **GPUパイプライン**（シェーダーやレンダリング設定を組み合わせた描画手順の定義）: ジオメトリパイプライン（unified, bezierPath）、フルスクリーンパイプライン（blit, composite, filter）の生成と保持
...
| キャッシュ | 内容 | キー |
|---|---|---|
| GeometryCache | パステッセレーション結果（頂点バッファ） | elementId |
| StrokeCache | ストロークメタデータ | elementId:strokeIndex |
| StencilFillCache | ファンベースフィルのステンシルデータ | elementId |
...
| シェーダー | 用途 |
|---|---|
| unified.wgsl | ジオメトリレンダリング（ストローク/フィル共通） |
