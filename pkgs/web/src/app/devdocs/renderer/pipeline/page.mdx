...
| `DocumentCache` | ステンシル・composite・prebuffer・final-blit・backdrop-mask用テクスチャの遅延生成と再利用 |
...
フィルター処理やグループ合成、クリップグループのステンシルマスキングでは、最終的なキャンバスに直接描画するのではなく、一時的なテクスチャに描画してから結果を合成する必要がある。たとえば、ぼかしフィルターを適用するには、まず要素をオフスクリーンテクスチャに描画し、そのテクスチャにフィルター処理を施してからキャンバスにblitする。`OffscreenPresenter` がこのオフスクリーンレンダーパスの生成とライフサイクル管理を担当する。

`createOffscreenPass()` が中核のプライベートメソッドで、指定されたワールド空間バウンズに対応するオフスクリーンテクスチャ・ステンシルテクスチャ・レンダーパスを生成する。テクスチャサイズはバウンズにビューポートzoomを乗じて算出し、GPU最大テクスチャサイズでクランプする。`TexturePool` から取得したテクスチャは128px単位量子化（SIZE_QUANTUM: サイズを128pxの倍数に切り上げること）されるため、実際のコンテンツ領域をUV矩形（`blitUvRect`）で切り出す。
...
ステンシル、composite、prebuffer、final-blit、backdrop-mask用テクスチャの遅延生成と再利用を管理する。名前に反して描画済みドキュメント内容やタイルを保持するキャッシュではない。

3つの `ensure*` メソッドを提供する:
- `ensureStencilTexture()` — `depth24plus-stencil8` フォーマットのステンシルテクスチャ
...
2つの関数を提供する:
- `createGeometryPipeline()` — 頂点バッファを持つパイプライン（stroke, fill, gradient, stencil系）。`GPUVertexBufferLayout` の単一/複数指定、トポロジ、ブレンド、デプスステンシル、マルチサンプル等をオプションで受け取る
