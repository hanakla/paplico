...
| ディレクトリ | モジュール | 役割 |
|---|---|---|
| elements/ | `ElementRenderer` | 要素種別ディスパッチ。パスの塗りと線は `PathElementRenderer` が被覆率 strip で描く |
...
flowchart TD
    ...
    E1 -->|gradient| E3["GradientRenderer.acquirePaintBindGroup で BG2 を用意し strip で塗る"]
```
...
4. **フィル描画**: 非ゼロ巻き数の被覆率を CPU で求め、strip としてインスタンス描画する。グラデーションの場合はペイント用の bind group を `GradientRenderer` から受け取る
...
パスのフィルには単色だけでなくグラデーション（色が連続的に変化する塗り）を指定できる。`GradientRenderer` はこのグラデーションのペイント bind group（BG2）を組み立てるモジュールである。`acquirePaintBindGroup` が唯一の公開メソッドで、strip 描画がそのまま BG2 に束ねて塗る。
