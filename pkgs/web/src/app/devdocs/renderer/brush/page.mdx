...
`wet.enabled`のブラシはにじみ・拡散のシミュレーションを通る。4フィールドの拡散シミュレーションで、係数はDab属性と場テクスチャから供給する（`pipeline/stroke/WetLayerPass.ts`）。
...
- `getSampler()`: リボン経路用。`lodMaxClamp: 0`でレベル0固定とし、リボンをフル解像度で描く
