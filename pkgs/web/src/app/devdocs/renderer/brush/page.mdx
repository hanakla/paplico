...
flowchart TD
    ...
    B -->|geometric| E[PathElementRenderer.renderGeometricStroke]
    C --> F[インスタンスド描画]
    D --> F
```
...
- **geometric**: SVG風のジオメトリックストローク。`PathElementRenderer.renderGeometricStroke`がテッセレーションして描く。`BrushRenderer`には来ない
...
| `MixPass` / `MixStrokeRenderer` | 混色（smudge）のチャンクcomputeとインライン合成。`MixStrokeRenderer`はCanvasLayerが生成する |
| `BlurStrokeRenderer` | 下地ぼかしストロークのインライン合成。CanvasLayerが生成する |
...
pathIndex/テクスチャレイヤのビット詰めは`StampPacking.ts`の純関数（`packStampPathIndex` / `replaceStampPathIndex`）が担う。この24 float ABIはフェーズ1で凍結されており、以後変更しない。wet無効のブラシもwet系4スロット（`wetness` / `directionality` / `grainAmount` / `packedWetCoefficients`）を0で運ぶが、これは固定ABIの意図的な代償である。

## DabEvaluator

`evaluateDabs(segments, settings, options)`がDab生成の本体である。`settings.engine !== "dab"`なら空バッファを返す。

- **入力**: `CubicBezierSegment[]`、`BrushSettings`、`DabEvaluateOptions`（`pathIndex` / `pathStart` / `pathEnd` / `strokeWidths` / `strokeWidthsBaked` / `textureAspectRatio` / 散布レイヤ情報 / `resume` / `totalLength`）
...
`strokeWidthsBaked`がtrueで`strokeWidths`がある場合、`strokeWidths`はsizeカーブの評価結果そのものである。このとき`neutralizeSizeCurves`でsizeカーブを外して評価する。プロファイルの左右の平均で`sizeX`と`sizeY`をスケールする。左右の差は法線方向のオフセットとしてDab位置へ反映する。falseの場合は`side1Width` / `side2Width`へそのまま書き、sizeカーブの評価に重ねる。
...
`DabEvalState`（速度EMA・接触速度の下限・累積距離・乱数状態など24フィールド）を`resume`オプションで受け渡すと、ストロークを分割評価しても一括評価とビット一致する。条件は2つある。全チャンクへ同じ`totalLength`を渡すこと。最初のチャンクが接触速度の窓（`CONTACT_SPEED_WINDOW_MS` = 200ms）を越えていること。速度の下限は最初の評価が見たセグメント列から求めるためである。`LiveDabAccumulator`の増分評価と混色のチャンク処理はこの性質に依存している。乱数はmulberry32のシード付きで、同一シードから同一Dab列が再現される。
...
flowchart TD
    ...
    B -->|ライブ wash / 全長依存のライブ buildup| D[毎フレーム evaluateDabs 全評価]
...
- **ライブwash / 全長依存のライブbuildup**: 毎フレーム全評価してフレームプールのバッファへアップロードする。buildupでも、終端tip（`endSource`）・`along`グラデーション・`strokeT`カーブのどれかがあると、評価済みの部分がストロークの伸びで変わる。`canAccumulateLiveDabs`がこれを判定する
...
- ビルトインは`BUILTIN_BRUSH_IDS`の7種: `svg` / `hardCircle` / `softCircle` / `pencil` / `airbrush` / `calligraphy` / `bacon`。pencil / airbrush / baconはbase64 PNGから、円形2種はプログラム生成（`BUILTIN_TEXTURE_SIZE = 128`）
...
washストロークは`RenderPlanner`がwashプランとして分離し、CanvasLayerの`renderIsolatedAppearances()`がオフスクリーンで描いてから合成する。グループ内の要素は`OffscreenPresenter`が`renderIsolatedWashAppearances`コールバック経由で同じメソッドを呼ぶ。確定ストロークの合成結果はwashResultCacheで再利用される。キャッシュに必ず収まるよう、ドメインは1辺4096pxでキャップされる。ライブプレビューのwashは毎フレーム描き直すためキャッシュしない。`resolveTransientWashDomain`でドメインをビューポートへクリップし、解像度の上限を表示密度にする。
...
## 下地ぼかし（BlurStrokeRenderer）

`backdropBlur.enabled`のブラシは絵の具を置かず、ストロークの下にある合成結果をぼかして見せる。混色と同じく下地を読むため、`BlurStrokeRenderer`が`BackdropEffectDriver`としてz順のインライン合成を行う。

1. Dab列を`DabRenderer.render`でcoverageテクスチャへ描く。色は使わずアルファだけを使う
2. `BackdropEffectCoordinator.acquireFixedRRegion`で固定R格子の下地を取得し、`sampleBlur(sigma)`で共有ピラミッドからsigmaを挟む2レベルを受け取る。sigmaは`backdropBlur.radius × ブラシサイズ / 2`をR倍したものである
3. `blurStroke.wgsl`のフルスクリーンパスが、2レベルを補間したぼかし画をcoverageで乗算して結果テクスチャへ書く
4. 結果は`BlitLayer`として返り、`coverage`にcoverageテクスチャを載せる。CanvasLayerはcoverage分だけ下地を打ち抜いてから合成するので、coverageが1の場所ではぼけた下地がそのまま置き換わる

同じブラシで`mixing`や`wet`が有効でも、`resolveBrushRenderRequirements`はぼかしを優先して両者をfalseにする。ぼかしは色を持たないため、混ぜる色も広げる顔料もないからである。

`prepareFrame`で当該フレームの全ぼかしストロークの要求を`planFrame`に登録し、共有バッチのピラミッドが最大sigmaを賄えるようにする。ライブプレビューなど登録前に捕捉されたバッチにあたった場合は、バッチを捨てて自分を対象に再捕捉する。

## wet層（WetLayerPass）

`wet.enabled`のブラシはにじみ・拡散のシミュレーションを通る。発展させる場はpigmentとmoistureの2つである。pigmentは`rgb = 色 × 濃度`、`a = 濃度`を持つ。moistureは`rg = 運ばれる流れ`、`b = 水量`、`a = pooling`を持つ。1反復で水の拡散、pigmentの移流と拡散、乾燥を進める。fluidVelocityのseedと係数テクスチャは反復中に書き換えない。係数はDab属性からseedへ書き込む。実装は`pipeline/brush/WetLayerPass.ts`である。
...
2. **field seed**（`wetLayerSeed.wgsl.ts`、compute）: seedをpigment / moistureの初期状態へ変換する。場はseedより`scale`倍粗い格子で走る。場の1texelは、対応する`scale × scale`のseed texelの平均を取る
3. **diffuse**（`wetLayerDiffuse.wgsl.ts`、compute）: pigment / moistureをping-pongで発展させる。反復数は32を下限、256を上限にbleedの到達距離から決まる。seedと係数テクスチャは全反復でread-onlyの固定バインドである
4. **finish**（`wetLayerFinish.wgsl.ts`）: 拡散結果を可視色へ変換して合成する。合成パスは、ドメインが覆うターゲット上の矩形へscissorする。ドメインがターゲットに掛からない場合は合成パスを発行しない

拡散の到達距離は格子間隔と反復数の組で決まる（`resolveWetBleedReach` / `resolveWetFieldScale` / `resolveWetIterations`）。格子間隔は、ブラシ直径に12セル以上が残る範囲で粗くする。それで足りない距離は反復数で補う。ステンシル幅を広げると格子が独立サブラティスに割れてブロブ状になるため、必ずこの組み合わせで距離を稼ぐ設計である。

駆動は`WetStrokeRenderer`が行う。入口は2つで、どちらも同じ非公開の`simulate`へ入る。`simulate`は`resolveSimulationDomain`で効果領域をタイルへ分ける。係数のclear valueはストロークごとに1回作る。値はストロークの各係数のbaseである。Dabが覆わないtexelもこの係数を持つ。タイルごとの処理の順序は次のとおりである。

1. seed textureを確保する
2. タイル用uniformを取得する
3. `DabRenderer.renderWetSeeds()`を呼ぶ
4. `WetLayerPass.apply()`を呼ぶ
5. seedを解放待ちへ移す

- `renderAppearance`: 通常水彩。CanvasLayerの`renderIsolatedAppearances`から呼ばれ、パスの合成済みtransformを効果領域へ適用し、isolation boundsに合わせた出力テクスチャを確保して`RasterizedRenderSurface`を返す。スケールにはアキュムレータのキャップを反映した`effectiveZoom`を受け取る。isolation全体を覆う出力テクスチャが、GPUのテクスチャ上限を超えないようにするためである
...
| wet seed MRT 6枚 | `WetStrokeRenderer`が確保し`DabRenderer.renderWetSeeds()`が描く | rgba16float×3 + rg8unorm×2 + r8unorm | `WetLayerPass`のfield seed・diffuse・finishの入力。反復中はread-only固定バインド | タイル処理内。フレームのsubmit後に解放 |
| wet拡散ping-pong 4枚 | `WetLayerPass` | rgba16float | pigment / moistureのA・B。diffuse反復の読み書き交互 | `WetLayerPass`が保持して再利用する。必要サイズが増えたときだけ作り直す |
