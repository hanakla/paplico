...
ブラシ設定は`BrushSettingsV2`（`core/schema.ts`）である。`engine`フィールドが描画エンジンを決め、`properties`のカーブ行列（入力×プロパティのカーブ変調）がDabごとのサイズ・不透明度・回転などを決める。カーブ評価の仕組み自体は`core/brush/`（`properties.ts` / `inputs.ts` / `curves.ts` / `evaluateProperties.ts`）にあり、本ページではレンダラがそれをどう消費するかに絞る。

## エンジンルーティング

`resolveBrushRenderRoute()`（`core/brush/renderRoute.ts`）が`BrushSettingsV2.engine`から3経路を決める。

```mermaid
flowchart TD
    A[Path + BrushSettingsV2] --> B[resolveBrushRenderRoute]
...
ルートを決めるのは呼び出し側である。CanvasLayer（バッチ経路）と`PathElementRenderer`（即時経路）が`resolveBrushRenderRoute()`を1回呼び、stroke appearanceから取り出した`strokeColor`と routed settings を`StrokeDrawInput`に詰めて`StrokeBatchContext`へ直接渡す。`StrokeBatchContext`は`path.filters`を読み直さない。
...
| `DabEvaluator` | セグメント列 + `BrushSettingsV2` → Dabインスタンス列。速度EMA・カーブ評価・timed dab |
...
- **入力**: `CubicBezierSegment[]`、`BrushSettingsV2`、`DabEvaluateOptions`（`pathIndex` / `pathStart` / `pathEnd` / `strokeWidths` / `textureAspectRatio` / 散布レイヤ情報 / `resume` / `totalLength`）
...
- `RibbonOptions.curved`に`BrushSettingsV2`を渡すと、sizeとflowのカーブがリボンにも効く。評価はセグメント端点で行い、間は線形補間する
