...
エンジンの選択はStrokeEngineRegistry（`pipeline/stroke/StrokeEnginePicker.ts`）が仲介する。`StampStrokeEngine` / `RibbonStrokeEngine` / `GeometricStrokeEngine`はいずれも薄いラッパで、実体は`StrokeBatchContext`に集約されている。
