...
describe("StrokeBatchContext", () => {
	...
	describe("same-frame conflict fallback", () => {
		...
		it("should regenerate a resident-baked stroke at pooled meta index 0, keeping its texture layers", () => {
			...
			// Immediately re-render "b" in the SAME frame with a different alpha
			// (the editing-scope dim overlay pattern): its lease was already
			// synced this frame, the meta rewrite conflicts, and the immediate
			// path falls back to pooled carrying a resident-BAKED buffer — the
			// regeneration branch, not the direct upload.
			context.render(passEncoder, bPath, 0.5);

			// a+b are store-adjacent residents (one merged draw); the conflicting
			// re-render follows through the pooled path — nothing is dropped and
			// draw order holds.
			expect(draws()).toEqual([
				{
					source: RESIDENT_STAMPS,
				...
				{ source: POOLED_STAMPS, instanceCount: count, firstInstance: 0 },
			]);

			const residentB = writes.filter(
				(w) => w.buffer.label === RESIDENT_STAMPS,
			)[1]?.data;
			const pooled = writes.find((w) => w.buffer.label === POOLED_STAMPS)?.data;
			if (!residentB || !pooled) {
				throw new Error("test setup: missing resident or pooled upload");
			}

			// Fixture guards: b's resident bake wrote a non-zero meta index and
			// the array brush packed some non-zero layer — otherwise the
			// assertions below would pass vacuously.
			expect(metaIndicesOf(residentB).some((v) => v !== 0)).toBe(true);
			expect(layersOf(residentB).some((v) => v !== 0)).toBe(true);

			// The regenerated pooled stamps reset the meta bits to the per-draw
			// index 0 while reproducing the exact texture-array layer packing.
			expect(metaIndicesOf(pooled).every((v) => v === 0)).toBe(true);
			expect(layersOf(pooled)).toEqual(layersOf(residentB));
		});
	});
