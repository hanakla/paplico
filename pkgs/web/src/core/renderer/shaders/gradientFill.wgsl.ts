...
export const GRADIENT_FILL_SHADER = /* wgsl */ `
struct Uniforms {
	viewportX: f32,
	viewportY: f32,
	zoom: f32,
...
struct GradientUniforms {
	...
	stopCount: u32,
	meshFaceCount: u32,
	// StrokeGradientMode (see unified.wgsl). This shader only supplies the
	// uniform layout for the shared gradient bind group; the field must stay
	// in the same slot as unified.wgsl's GradientUniforms.
...
fn vs_main(in: VertexInput) -> VertexOutput {
	...
	let worldPos = transformed + in.offset;
	...
	out.color = in.color;
	// Pass pre-transform position for gradient UV calculation.
	// CPU-side boundsMin/boundsMax are in local (pre-transform) space.
	out.worldPos = in.position + in.offset;
	return out;
}
...
fn sampleMeshGradient(uv: vec2f) -> vec4f {
	...
	var color: vec4f;
		...
		switch gradient.gradientType {
			// Solid: use vertex color directly
			case 0u: {
				let solidA = in.color.a;
				return vec4f(in.color.rgb * solidA, solidA);
			}
		// Linear gradient
		case 1u: {
		...
		// Free-form gradient
			...
			// Pattern fill: handled by the unified geometry shader. The
			// standalone gradient-fill pipeline never dispatches a pattern, so
			// fall through to a transparent sample to keep WGSL switch
			// exhaustiveness happy without altering existing draws.
			case 5u: {
				color = vec4f(0.0);
			}
			default: {
				let fallbackA = in.color.a;
				return vec4f(in.color.rgb * fallbackA, fallbackA);
			}
		}
