/**
 * Paint stage shared by every document-geometry shader: the BG2 paint
 * bindings (gradient uniforms, colour stops, free/pattern texture, mesh
 * buffers) and `paintColor`, which resolves one fragment's premultiplied
 * colour from the `gradientType` discriminator.
 */

import { GRADIENT_COMMON_WGSL } from "./gradientCommon.wgsl";

export const PAINT_COMMON_WGSL = /* wgsl */ `
struct GradientUniforms {
	gradientType: u32,
	stopCount: u32,
	meshFaceCount: u32,
	// StrokeGradientMode for geometric strokes: 0 = within (bounds-space),
	// 1 = along (sample stops at param t), 2 = across (param u). Always 0
	// for fills.
...
/**
 * Premultiplied paint colour of one fragment. solid is the straight RGBA
 * used by solid paint; its alpha is the opacity multiplier of every paint.
 * tu carries the along/across stroke gradient params.
 */
fn paintColor(worldPos: vec2f, solid: vec4f, tu: vec2f) -> vec4f {
	let opacity = clamp(solid.a, 0.0, 1.0);
	let boundsSize = gradient.boundsMax - gradient.boundsMin;
	let uv = (worldPos - gradient.boundsMin) / boundsSize;

	var color: vec4f;

	switch gradient.gradientType {
		// Solid: the straight colour, premultiplied
		case 0u: {
			return vec4f(solid.rgb * solid.a, solid.a);
		}
		// Linear gradient
		case 1u: {
			if gradient.strokeGradientMode == 1u {
				color = sampleGradientStops(tu.x);
			} else if gradient.strokeGradientMode == 2u {
				color = sampleGradientStops(tu.y);
			} else {
	...
	return vec4f(color.rgb * outA, outA);
}
`;
