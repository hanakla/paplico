...
describe("PaplicoCommands.createPatternDefFromSelection", () => {
	it("returns null when nothing is selected", () => {
		const { commands } = setup([], []);
		expect(commands.createPatternDefFromSelection("pat")).toBeNull();
	});

	it("clones selected elements with fresh ids, centers them on origin, and creates a pattern def", () => {
		...
		const defId = commands.createPatternDefFromSelection("my-pattern");
